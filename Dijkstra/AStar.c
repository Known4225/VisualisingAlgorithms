...
typedef struct {
    ...
    int nodeCount;
    int selected;
    int start; // start and end nodes for the A* algorithm
    int end;
    int stepNum;
    int wireStart;
    int wireEnd;
...
double heuristic(AStar *selfp, int node1, int node2) { // a simple distance heuristic
    AStar self = *selfp;
    double x = ((Node *) self.nodes -> data[node1].p) -> xpos - ((Node *) self.nodes -> data[node2].p) -> xpos;
    double y = ((Node *) self.nodes -> data[node1].p) -> ypos - ((Node *) self.nodes -> data[node2].p) -> ypos;
    return sqrt(x * x + y * y);
}
...
void setupAStar(AStar *selfp) {
    AStar self = *selfp;
    for (int i = 0; i < self.nodes -> length; i++) { // load heuristic values
        ((Node *) self.nodes -> data[i].p) -> heuristic = heuristic(&self, i, self.end);
    }
    if (self.start != -1 && self.end != -1) {
        ...
        list_append(self.queue, (unitype) (0.0 + ((Node *) self.nodes -> data[self.start].p) -> heuristic), 'd'); // path distance + heuristic
    ...
    if (self.queue -> length > 0 && !self.finishedAStar) {
        ...
        for (int i = 0; i < head.neighbors -> length; i += 2) {
            ...
            if (visited) { // is in queue
                double h = ((Node *) self.nodes -> data[head.neighbors -> data[i].i].p) -> heuristic;
                if (self.queue -> data[1].d + head.neighbors -> data[i + 1].d < self.queue -> data[visited + 1].d) { // if new path is shorter than old path
                    ...
                    self.queue -> data[visited + 2] = (unitype) (self.queue -> data[1].d + head.neighbors -> data[i + 1].d + h);
                    self.queue -> data[visited + 3] = self.queue -> data[0];
                    int j = visited + 1;
                    while (self.queue -> data[j + 1].d < self.queue -> data[j - 3].d) { // swap adjacents (based on path distance + heuristic)
                ...
                if (!visited) { // is not in queue or completed lists
                    ...
                    list_append(self.queue, (unitype) (self.queue -> data[1].d + head.neighbors -> data[i + 1].d + h), 'd');
...
void mouseTick(AStar *selfp) {
    ...
    if (turtleMouseDown()) {
        ...
        if (self.keys[0] == 1) {
            ...
            if (self.selected == -1 && fabs(self.focalCSX - self.screenX) < 0.01 && fabs(self.focalCSY - self.screenY) < 0.01 && self.stepNum == 0) {
                self.toggle = 0;
                self.start = -1;
                self.end = -1;
                list_clear(self.queue);
...
int main(int argc, char *argv[]) {
    ...
    window = glfwCreateWindow(1280, 720, "A* Algorithm", NULL, NULL);
    if (!window) {
        glfwTerminate();
    }
