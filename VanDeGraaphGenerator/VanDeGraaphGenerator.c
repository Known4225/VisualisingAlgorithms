...
void init(Graph *selfp, int nodeCount) {
    Graph self = *selfp;
    ...
    for (int i = 0; i < nodeCount; i++) {
        ...
        list_append(self.size, (unitype) (100 / (log(nodeCount) + 1)), 'd');
        int tries = 1;
        char colliding = 1;
    ...
    for (int totalTries = 0; totalTries < nodeCount * nodeCount / 4; totalTries++) { // the total number of attempted connections is nodeCount squared upon 4
        ...
        if (i != j) {
            ...
            for (int k = 0; k < self.connections -> length; k += 2) {
...
void renderGraph(Graph *selfp) { // renders the nodes
    Graph self = *selfp;
...
void renderConnections(Graph *selfp) { // renders the connections between nodes
    Graph self = *selfp;
    turtlePenSize(5 / (log(self.xpos -> length) + 1) * self.screenSize);
    for (int i = 0; i < self.connections -> length; i += 2) {
...
void snapToGrid(Graph *selfp, double gridSize) { // the classic n^2 snap to grid algorithm, one day I'm going to solve this in O(n) you just wait, and it will be coke milk
    Graph self = *selfp;
    int modMedX = 0;
    int modMedY = 0;
    double minDistX = 1000000000.0;
    double minDistY = 1000000000.0;
    for (int i = 0; i < self.xpos -> length; i++) { // the theorem states that for any set of real numbers, there is one that is at the minimum distance mod n from all the other nodes, known as the modular median, this n^2 algorithm finds the modular median, but it is conjectured it could be done in O(n) like a regular median can
        double accDistX = 0;
        double accDistY = 0;
...
void mouseTick(Graph *selfp) {
    ...
    if (self.selectMode == 0 || self.selectMode == 4) {
        ...
        for (int i = 0; i < self.xpos -> length; i++) {
            if ((self.mouseX - (self.xpos -> data[i].d + self.screenX) * self.screenSize) * (self.mouseX - (self.xpos -> data[i].d + self.screenX) * self.screenSize) + (self.mouseY - (self.ypos -> data[i].d + self.screenY) * self.screenSize) * (self.mouseY - (self.ypos -> data[i].d + self.screenY) * self.screenSize) < self.size -> data[i].d * self.size -> data[i].d * 0.25 * self.screenSize * self.screenSize) {
...
int main(int argc, char *argv[]) {
    ...
    window = glfwCreateWindow(1280, 720, "VanDeGraaphGenerator™", NULL, NULL);
    if (!window) {
        glfwTerminate();
    }
